...
function ActivityCard({ item, pipelineValue, onUpdate, onDelete }) {
  const [editing, setEditing] = useState(false);
  const [title, setTitle] = useState(item.title);
  const [url, setUrl] = useState(item.url);
  const isPipeline = item.id === 'pipeline';

  const save = () => { onUpdate(item.id, title, url); setEditing(false); };
    ...
    <div style={{background:'#fff',border:'1px solid #E0E4EC',borderRadius:12,padding:'14px 16px',marginBottom:8}}>
      {editing ? (
        <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
          ...
          <button onClick={()=>{ setTitle(item.title); setUrl(item.url); setEditing(false); }} style={{fontSize:12,padding:'6px 10px',borderRadius:6,background:'#F4F6F9',border:'1px solid #DDE1E8',color:'#3A4A5C',cursor:'pointer'}}>Cancel</button>
        </div>
      ) : (
...
function ActivityBoard({ items, pipelineValue, onAdd, onUpdate, onDelete }) {
  ...
  return (
    <div style={{marginTop:18}}>
      ...
      <p style={{fontSize:12,color:'#8A9BB0',margin:'0 0 12px'}}>Where to look for more — pipeline, leads, AMC accounts, competitor accounts, cold prospecting. Add your own links, or use this as a checklist when planning your day.</p>
      {items.map(item=>(
        <ActivityCard key={item.id} item={item} pipelineValue={item.id==='pipeline'?pipelineValue:null} onUpdate={onUpdate} onDelete={onDelete}/>
      ))}
...
export default function Dashboard() {
  ...
  const updateActivity = async (id, title, url) => {
    try {
      const res = await fetch('/api/activities', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({action:'update', id, title, url})});
      const data = await res.json();
      if (data.items) setActivityItems(data.items);
    } catch {}
  };
        ...
        <div>
          ...
          {growthSM && (
            <>
              ...
              <ActivityBoard items={activityItems} pipelineValue={growthPipelineValue} onAdd={addActivity} onUpdate={updateActivity} onDelete={deleteActivity}/>
            </>
          )}
        </div>
      )}
