...
// Simple per-person login — name dropdown + password. First-ever login for
// a given name sets their password (no separate admin setup step needed);
// every login after that checks against it. Gates the entire dashboard, and
// the resulting session is what verifies who's reacting/commenting on the
// Closed tab (no more free-text "type any name you like").
function LoginScreen({ onLogin }) {
  const [name, setName] = useState(SM_LIST[0]);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (data.isNewAccount) setNotice(`Password set for ${name} — you're in.`);
  ...
  return (
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',background:'#F4F6F9',fontFamily:'-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,sans-serif'}}>
      <form onSubmit={submit} style={{background:'#fff',border:'1px solid #E0E4EC',borderRadius:14,padding:'2rem',width:320}}>
        ...
        <label style={{fontSize:11,color:'#8A9BB0',display:'block',marginBottom:4}}>Name</label>
        <select value={name} onChange={e=>setName(e.target.value)} style={{width:'100%',fontSize:13,padding:'8px 10px',border:'1px solid #DDE1E8',borderRadius:8,marginBottom:14}}>
          {SM_LIST.map(n=><option key={n} value={n}>{n}</option>)}
        </select>
        ...
        <p style={{fontSize:10,color:'#8A9BB0',margin:'0 0 16px'}}>First time signing in as this name? Whatever you enter becomes your password from now on.</p>
...
export default function Dashboard() {
  ...
  // for reactions/comments (replaces the old free-text name field entirely) ----
    ...
    (async () => {
      ...
      if (t) {
        try {
          ...
          const data = await res.json();
          if (data.name) { setAuthToken(t); setAuthName(data.name); }
          else { localStorage.removeItem('dashboard_auth_token'); }
        } catch {}
      }
      setAuthChecked(true);
    })();
  ...
  const handleLogin = (name, token) => {
    setAuthName(name); setAuthToken(token);
  ...
  const handleLogout = async () => {
    ...
    setAuthName(null); setAuthToken(null);
      ...
      {tab==='closed'&&(
        <div>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12,flexWrap:'wrap'}}>
            <span style={{fontSize:12,color:'#3A4A5C'}}>Logged in as <b style={{color:'#0A1628'}}>{authName}</b></span>
            <button onClick={handleLogout} style={{fontSize:11,padding:'3px 10px',borderRadius:6,background:'#F4F6F9',border:'1px solid #DDE1E8',color:'#3A4A5C',cursor:'pointer'}}>Log out</button>
