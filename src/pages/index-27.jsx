...
export default function Dashboard() {
  ...
  // ---- Closed-tab extras: sound (mutable), confetti, and the social feed ----
  ...
  useEffect(()=>{ try{ setYourName(localStorage.getItem('dashboard_your_name')||''); }catch{} },[]);
  ...
  const saveName = (n) => {
    setYourName(n);
    try{ localStorage.setItem('dashboard_your_name', n); }catch{}
  };
  ...
  const closedData = useMemo(()=>{
    ...
    return { thisWeek, lastWeek, thisWeekDeals, leaderboard, reigningChampion, championPostId, ytdDealsByBrand, year };
  }, [deals]);
  ...
  const reactTo = async (postId, emoji) => {
    ...
    if (!name) { alert('Enter your name in "Posting as" first, so others can see who reacted.'); return; }
    setSocialData(prev=>{
    ...
    try {
      await fetch('/api/social', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({postId, action:'react', emoji, name})});
    } catch {}
  };

  const commentOn = async (postId, text) => {
    const name = yourName || 'Someone';
    setSocialData(prev=>{
      const cur = prev[postId] || {reactions:{},comments:[]};
      return {...prev, [postId]: {...cur, comments:[...cur.comments, {name, text, ts:Date.now()}]}};
    });
    try {
      await fetch('/api/social', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({postId, action:'comment', name, text})});
    } catch {}
  };
      ...
      {tab==='closed'&&(
        <div>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:12,flexWrap:'wrap'}}>
            <label style={{fontSize:12,color:'#8A9BB0'}}>Posting as</label>
            <input value={yourName} onChange={e=>saveName(e.target.value)} placeholder="Your name"
              style={{fontSize:12,padding:'4px 8px',border:'1px solid #DDE1E8',borderRadius:6,width:160}}/>
