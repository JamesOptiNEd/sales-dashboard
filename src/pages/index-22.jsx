...
function YtdClosedSummary({ brand, color, monthly, year }) {
  const [open, setOpen] = useState(false);
...
export default function Dashboard() {
  ...
  const closedData = useMemo(()=>{
    ...
    // YTD monthly/quarterly totals (Closed Won, current year, total deal value) — split by brand
    const year = today.getFullYear();
    const buildMonthly = (brand) => {
      const m = {};
      for (let i=0;i<12;i++) m[i] = 0;
      for (const d of deals) {
        if (d.stage!=='Closed Won' || d.closeyear!==year || d.closemonth==null || d.brand!==brand) continue;
        m[d.closemonth] += d.totalValue || 0;
      }
      return m;
    };
    const monthly = { Optitex: buildMonthly('Optitex'), NedGraphics: buildMonthly('NedGraphics') };

    const championPostId = `champion-${lastWeek.start.toISOString().slice(0,10)}`;

    return { thisWeek, lastWeek, thisWeekDeals, leaderboard, reigningChampion, championPostId, monthly, year };
  }, [deals]);
    ...
    <div style={{fontFamily:"'Inter',-apple-system,sans-serif",padding:'1.5rem',maxWidth:1200,margin:'0 auto'}}>
      ...
      {tab==='closed'&&(
        <div>
          ...
          <YtdClosedSummary brand="Optitex" color={NAV} monthly={closedData.monthly.Optitex} year={closedData.year}/>
          <YtdClosedSummary brand="NedGraphics" color={TEAL} monthly={closedData.monthly.NedGraphics} year={closedData.year}/>
        </div>
      )}
