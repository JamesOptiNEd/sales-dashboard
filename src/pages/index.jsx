...
function AddResourceForm({ onAdd, onCancel, defaultType='link' }) {
  const [title, setTitle] = useState('');
  const [type, setType] = useState(defaultType);
  const submit = () => { if (title.trim()) onAdd(title, type); };
  return (
    <div style={{display:'flex',gap:6,alignItems:'center',flexWrap:'wrap',marginTop:6,marginBottom:4}}>
      <input value={title} onChange={e=>setTitle(e.target.value)} placeholder="Name…" autoFocus
        onKeyDown={e=>{ if(e.key==='Enter') submit(); if(e.key==='Escape') onCancel(); }}
        style={{fontSize:12,padding:'5px 8px',border:'1px solid #DDE1E8',borderRadius:6,flex:'1 1 140px'}}/>
      <select value={type} onChange={e=>setType(e.target.value)} style={{fontSize:12,padding:'5px 8px',border:'1px solid #DDE1E8',borderRadius:6}}>
        <option value="link">Link</option>
        <option value="folder">Folder</option>
      </select>
      <button onClick={submit} style={{fontSize:12,padding:'5px 12px',borderRadius:6,background:NAV,color:'#fff',border:'none',cursor:'pointer'}}>Add</button>
...
// One node in the resource tree — folder or link, rendered recursively.
// Folders expand/collapse; links show an Open button once a URL is set.
      ...
      {isFolder && open && (
        <div style={{marginTop:4}}>
          ...
          <div style={{marginLeft:18}}>
            {adding ? (
              <AddResourceForm onAdd={(t,ty)=>{ onAdd(node.id, t, ty); setAdding(false); }} onCancel={()=>setAdding(false)}/>
            ) : (
              <button onClick={()=>setAdding(true)} style={{fontSize:12,padding:'5px 12px',borderRadius:6,background:'#F4F6F9',border:'1px dashed #C7CEDA',color:'#3A4A5C',cursor:'pointer',marginTop:4}}>+ Add</button>
            )}
          </div>
        </div>
      )}
...
function ResourceRootAdd({ onAdd }) {
  const [adding, setAdding] = useState(false);
  return adding ? (
    <AddResourceForm defaultType="folder" onAdd={(title,type)=>{ onAdd(null, title, type); setAdding(false); }} onCancel={()=>setAdding(false)}/>
  ) : (
