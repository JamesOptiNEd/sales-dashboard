...
const OPTITEX_SMS = ['Sabine','Marco','Giulia','Susanna'];
const NED_SMS = ['Canan','Henk','Susanna'];
  ...
  return (
    <div style={{background:bg,borderRadius:12,padding:small?'12px 16px':'16px 20px',marginBottom:4}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
        <p style={{fontSize:small?11:12,color:'#4A6A8A',margin:0,textTransform:'uppercase',letterSpacing:'0.5px',fontWeight:500}}>{label}</p>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'1rem'}}>
        <div>
          <p style={{fontSize:10,color:'#4A6A8A',margin:'0 0 3px',textTransform:'uppercase',letterSpacing:'0.4px'}}>Target</p>
          <p style={{fontSize,fontWeight:600,margin:0,color:'#fff'}}>{fmtK(target)}</p>
        </div>
        <div>
          <p style={{fontSize:10,color:'#4A6A8A',margin:'0 0 3px',textTransform:'uppercase',letterSpacing:'0.4px'}}>Achieved</p>
          <p style={{fontSize,fontWeight:600,margin:0,color:'#4AE89A'}}>{fmtK(achieved)}</p>
        </div>
        <div>
          <p style={{fontSize:10,color:'#4A6A8A',margin:'0 0 3px',textTransform:'uppercase',letterSpacing:'0.4px'}}>Balance to go</p>
          <p style={{fontSize,fontWeight:600,margin:0,color:balColor(balance,target)}}>{fmtK(balance)}</p>
        </div>
        <div>
          <p style={{fontSize:10,color:'#4A6A8A',margin:'0 0 3px',textTransform:'uppercase',letterSpacing:'0.4px'}}>Pipeline total</p>
...
const STORAGE_KEY='opti_dashboard_targets_v3';
  ...
  return (
    <div style={{marginBottom:16}}>
      ...
      {smOpen && smList.map(sm=>(
        <SMRow key={sm} sm={sm} deals={brandDeals} targets={targets} pipeStages={pipeStages}/>
      ))}
    </div>
  );
}

function SMRow({ sm, deals, targets, pipeStages }) {
  const [splitOpen, setSplitOpen] = useState(false);
