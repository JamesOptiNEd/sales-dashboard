...
export default function Dashboard() {
  ...
  // One-click nudge: opens a pre-filled email to the deal owner (pulled from
  // HubSpot) asking them to review/update the close date.
  const nudgeHref = (d, kind) => {
    ...
    return `mailto:${d.smEmail||''}?subject=${subject}&body=${encodeURIComponent(body)}`;
  };
                ...
                {hygieneData.overdue.map(d=>(
                  <tr key={d.id} style={{borderBottom:'1px solid #F0F2F6',background:'#FDF2F1'}}>
                    ...
                    <td style={{padding:'7px 10px',textAlign:'right'}}>
                      <a href={nudgeHref(d,'overdue')} style={{fontSize:11,padding:'3px 10px',borderRadius:6,background:RED,color:'#fff',textDecoration:'none',whiteSpace:'nowrap'}}>Nudge</a>
                    </td>
                  </tr>
                ))}
              ...
              <tbody>
                {hygieneData.closingSoon.map(d=>(
                  <tr key={d.id} style={{borderBottom:'1px solid #F0F2F6',background:'#FBF6EC'}}>
                    ...
                    <td style={{padding:'7px 10px',textAlign:'right'}}>
                      <a href={nudgeHref(d,'closingSoon')} style={{fontSize:11,padding:'3px 10px',borderRadius:6,background:AMBER,color:'#fff',textDecoration:'none',whiteSpace:'nowrap'}}>Nudge</a>
                    </td>
                  </tr>
                ))}
